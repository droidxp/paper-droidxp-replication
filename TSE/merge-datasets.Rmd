...
nrow(result)
write.csv(result, "result.csv", quote = F, row.names = F)

apphash <- read.csv("sampleAppHash.csv", head=T, sep=',')
apphash <- sqldf("select * from apphash")
nrow(apphash)

firstPart <- sqldf("select h.hash as sha256original, h2.hash as sha256,
              case
...
secondPart <- sqldf("select f.sha256original, f.sha256,f.apidetected,s.identical, s.similar, s.new, s.deleted, s.simiscore from firstPart f 
...
thirdPart <- sqldf("select se.sha256original, o.benign ,se.sha256, r.family, 
                    case
	                     when (r.family = 'none') THEN 'False'
	                  Else 'True'
                     END as malware,
               se.apidetected,se.identical, se.similar, se.new, se.deleted, se.simiscore from secondPart se 
               left join originalFamily o on o.hash = se.sha256original
               left join malwareFamily r on r.sha256repackage = se.sha256")

write.csv(thirdPart, "sample_final_ds_before_VT_check.csv", quote = F, row.names = F)
