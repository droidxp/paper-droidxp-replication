...
s1 <- sqldf("select r.app, h.sha256, h.md5, a.family, 'True' as Malware 
                   from repackaged r, hashes h, avclass a
                   where r.hash = h.sha256 and 
                         h.md5 = a.hash")

s2 <- sqldf("select r.app, h.sha256, h.md5, 'None' as family, 'False' as Malware 
                   from repackaged r, hashes h
                   where r.hash = h.sha256 and 
                         h.md5 not in (select a.hash from avclass a)")
