...
thirdStep <- sqldf("select s.*,r.identical,r.similar,r.new,r.deleted,r.similarity,m.vendormalicious from 
                   secondStep s 
                   inner join simiResults r on s.sha256 = r.sha256
                   inner join malwareFamily m on s.sha256 = m.sha256repackage")

nrow(thirdStep)
write.csv(thirdStep, "sample_final_ds.csv", quote = F, row.names = F)
